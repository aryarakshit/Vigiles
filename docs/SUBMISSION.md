...
- **Stylus size engineering.** The vault compresses to 23,763 B brotli against the 24,576 B EIP-170 ceiling. Getting a 19-function contract there took a limb-wise `U256/u64` division (replacing 4.4 KB of `ruint`), a division-free oracle floor (`minOut·D + D > N`), and building `core` with the `immediate-abort` panic strategy (removing ~10 KB of `core::fmt` that panics keep alive through function pointers).
